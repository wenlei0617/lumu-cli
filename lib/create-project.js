...
module.exports = function() {
  inquirer.prompt(question).then(answers => {
    ...
    if (mode === 'PC') {
      exec(`git clone https://github.com/wenlei0617/react-template.git ${name}`, (error) => {
        if (error) {
          console.log(error);
          line.red('下载模板失败')
          process.exit();
      ...
      exec(`git clone https://github.com/wenlei0617/react-template-webapp.git ${name}`, (error) => {
        if (error) {
          console.log(error);
          line.red('下载模板失败')
          process.exit();
